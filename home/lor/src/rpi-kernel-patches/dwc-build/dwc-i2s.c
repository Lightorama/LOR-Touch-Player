			  ...
			  struct snd_soc_dai *cpu_dai)
{
	...
	/* hw.info stays zero after soc_pcm_hw_init; set INTERLEAVED so the
	 * access constraint mask is non-zero and snd_pcm_open_substream succeeds.
	 * RP1 DMA is non-coherent so hw_support_mmap returns false; interleaved
	 * RW is the only access mode needed. */
