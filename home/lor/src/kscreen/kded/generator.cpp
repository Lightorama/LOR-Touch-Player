...
KScreen::ConfigPtr Generator::idealConfig(const KScreen::ConfigPtr &currentConfig)
{
    ...
    // Non-laptop devices with a built-in panel (e.g. DSI display) should also
    // have that panel as primary, not whichever output has the highest resolution.
    if (embeddedOutput(connectedOutputs)) {
        laptop(config);
        return fallbackIfNeeded(config);
    }
