...
bool ShellUtil::isSystem24HourFormat()
{
    ...
    if (localeSettings.hasKey("TimeFormat")) {
        QString timeFormat = localeSettings.readEntry("TimeFormat");
        return timeFormat.contains(QLatin1Char('H'));
    }

    // No explicit override — defer to the system locale (honours LC_TIME)
    return QLocale::system().timeFormat(QLocale::ShortFormat).contains(QLatin1Char('H'));
}
